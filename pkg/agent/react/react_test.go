...
func (m *mockAligner) InjectMessage(state domain.State, currentStep, maxStep int) {
	if m.injectMessageFunc != nil {
		m.injectMessageFunc(state, currentStep, maxStep)
	}
	// Default: no alignment message (do nothing)
}
...
func TestReAct_Invoke_ToolCall(t *testing.T) {
	...
	// Create mock aligner that injects system message after tool result (like ScenarioAligner)
	mockAlign := &mockAligner{
		injectMessageFunc: func(state domain.State, currentStep, maxStep int) {
			if lastMsg := state.GetLastMessage(); lastMsg != nil && lastMsg.Type() == message.MessageTypeToolResult {
				state.AddMessage(message.NewAlignerSystemMessage("You received a tool result. Use this result to answer the user's original question directly."))
			}
		},
	}
	...
	// Note: Aligner messages are now removed by CleanupMandatory() before final response
	messages := react.state.GetMessages()
	if len(messages) != 4 {
	...
	// Verify the message sequence (aligner message removed by CleanupMandatory)
	if messages[0].Type() != message.MessageTypeUser {
