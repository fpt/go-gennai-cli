...
# Utility script to extract only the response content from gennai output, excluding thinking blocks
...
#
# Returns: The content from the response header ("gennai (<model>)") to the turn delimiter, excluding thinking output
...
if [ -z "$turn_number" ]; then
    # Single response mode - extract from 'gennai (' header to delimiter
    sed -n '/gennai (/,/^────────────────────────────────────────────────────────────$/p' "$output_file" | \
