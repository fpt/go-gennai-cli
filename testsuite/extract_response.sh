...
#
# Returns: The content from "✅ Response:" to the turn delimiter, excluding thinking output
...
if [ -z "$turn_number" ]; then
    # Single response mode - extract the first/only ✅ Response: section
    sed -n '/✅ Response:/,/^────────────────────────────────────────────────────────────$/p' "$output_file" | \
