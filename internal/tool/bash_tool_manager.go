...
func (m *BashToolManager) registerBashTools() {
	...
	m.RegisterTool("bash", "Execute shell commands with timeout and error handling",
		[]message.ToolArgument{
			{
				Name:        "command",
				Description: "Shell command to execute (e.g., 'go build ./gennai', 'git status', 'ls -la')",
				Required:    true,
				Type:        "string",
			},
			{
	...
	// Legacy compatibility - go_build
	m.RegisterTool("go_build", "Build Go packages (legacy compatibility)",
		[]message.ToolArgument{
			{
				Name:        "package_path",
				Description: "Go package path to build (default: current directory)",
				Required:    false,
				Type:        "string",
			},
		},
	...
	// Legacy compatibility - go_run
	m.RegisterTool("go_run", "Run Go programs (legacy compatibility)",
		[]message.ToolArgument{
			{
				Name:        "package_path",
				Description: "Go package path to run",
				Required:    true,
				Type:        "string",
			},
			{
...
// Legacy go_build handler
func (m *BashToolManager) handleGoBuild(ctx context.Context, args message.ToolArgumentValues) (message.ToolResult, error) {
	packagePath := "."
	if path, ok := args["package_path"].(string); ok && path != "" {
		packagePath = path
	}

	command := fmt.Sprintf("go build %s", packagePath)
	result, err := m.executeCommand(ctx, command, "Build Go package")
	if err != nil {
		return message.NewToolResultError(err.Error()), nil
	}
...
// Legacy go_run handler
func (m *BashToolManager) handleGoRun(ctx context.Context, args message.ToolArgumentValues) (message.ToolResult, error) {
	packagePath, ok := args["package_path"].(string)
	if !ok {
		return message.NewToolResultError("package_path parameter is required"), nil
	}

	command := fmt.Sprintf("go run %s", packagePath)

	// Add arguments if provided
	if argsStr, ok := args["args"].(string); ok && argsStr != "" {
		command = fmt.Sprintf("%s %s", command, argsStr)
	}

	result, err := m.executeCommand(ctx, command, "Run Go program")
	if err != nil {
		return message.NewToolResultError(err.Error()), nil
	}
...
func (m *BashToolManager) executeCommand(ctx context.Context, command, description string) (string, error) {
	...
	if description != "" {
		logger.InfoWithIcon("🔧", "Executing command", "description", description, "command", command)
	} else {
		logger.InfoWithIcon("🔧", "Executing command", "command", command)
	}
