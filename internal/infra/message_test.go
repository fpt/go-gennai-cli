...
func TestMessageHistoryRepositoryComplexToolScenario(t *testing.T) {
	...
	// Add system message with aligner source
	alignerMsg := message.NewAlignerSystemMessage("Alignment guidance")
	messages = append(messages, alignerMsg)
	...
	// Verify aligner message
	if loadedMessages[0].Source() != message.MessageSourceAligner {
		t.Fatal("Aligner message source not preserved")
	}
