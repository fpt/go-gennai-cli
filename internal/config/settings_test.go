...
func TestCreateDefaultSettingsFile(t *testing.T) {
	...
	tempDir, err := os.MkdirTemp("", "gennai-settings-test")
	if err != nil {
	...
	settingsPath := filepath.Join(tempDir, ".gennai", "settings.json")
...
func TestLoadSettingsCreatesFileWhenNoneExists(t *testing.T) {
	...
	tempDir, err := os.MkdirTemp("", "gennai-home-test")
	if err != nil {
		t.Fatalf("Failed to create temp directory: %v", err)
	}
	defer func() {
	...
	expectedPath := filepath.Join(tempDir, ".gennai", "settings.json")
